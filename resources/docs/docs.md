...
The `home-routes` handler in the `coercion-poc.routes.home` namespace
...
The `home-page` function will in turn call the `coercion-poc.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `coercion-poc.handler` namespace:
...
Request middleware functions are located under the `coercion-poc.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `coercion-poc.dev-middleware` namespace found in
